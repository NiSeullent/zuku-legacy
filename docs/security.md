...
사용자 문자열은 HTML 이스케이프 후 출력한다. 원격 HTML을 그대로 삽입하지 않는다. 기본 응답은 `no-store`, `nosniff`, framing 제한 및 CSP를 제공하지만 IE6가 최신 헤더를 모두 이해한다고 가정하지 않는다. 미디어 요소는 `preload="none"`으로 자동 재생·진입 즉시 다운로드를 하지 않으며, 사용자가 재생하면 허용된 HTTPS URL을 브라우저의 native player 또는 구형 object/embed 경로로 전달한다. Legacy는 미디어 URL을 프록시하거나 클라이언트 암호화를 추가하지 않는다.
