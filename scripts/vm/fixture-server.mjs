...
const app = {
  async handle(request, context) {
    ...
    if (url.pathname === '/legacy/__qa/results' && request.method === 'POST') {
      ...
      if (result.length > 16000) return new Response('Report too large', { status: 413 });
      const directory = new URL('../../artifacts/', import.meta.url);
      await mkdir(directory, { recursive: true });
      await writeFile(new URL('ie6-vm-report.txt', directory), result);
      console.log('Saved explicit VM probe result to artifacts/ie6-vm-report.txt');
...
const server = createStandaloneServer({ app, publicOrigin });
