...
현대 ZUKU의 API를 Internet Explorer 6–11과 저사양 기기에 연결하는 초기 구현입니다. Internet Explorer는 모두 Classic 화면을 사용합니다. 전체 기능 동등성이나 모든 실제 기기의 호환성 인증을 뜻하지 않습니다. 이 저장소는 기존 프로젝트에 연결할 패키지와 예제를 제공하며, 운영 중인 모던 ZUKU 프로젝트를 변경하거나 자동 배포하지 않습니다.
...
공개 URL도 공유합니다. `/`는 홈, `/community`는 Thread이며 상품 채널·검색·콘텐츠·창작자 프로필도 기존 경로를 유지합니다. 보호된 연결에서는 `/login`으로 계정을 연결하고 `/profile`에서 내 계정을 엽니다. 평문 HTTP 화면은 사용할 수 없는 계정·보관함·메시지 메뉴와 양식을 표시하지 않습니다. IE6–11과 감지 결과가 명확한 구형 브라우저 요청만 내부 렌더러로 rewrite하며 주소창과 내부 링크에는 공개 경로를 유지합니다. `/legacy`는 정적 자산·action·승인 API와 명시적 디버깅용 내부 경로입니다.

현재 화면은 영상·음악의 HTML5 재생과 IE6–8용 object/embed·파일 열기 fallback을 제공하며, 변환 완료 playback URL을 우선 사용합니다. 실제 구형 엔진 재생은 설치된 codec/plugin 또는 서버 변환 결과에 따라 달라집니다. 게임은 소개와 대화를 제공하고 실행은 모던 화면으로 연결합니다. 메시징은 기존 E2EE를 유지하기 위해 모던 화면으로 연결합니다. 패스키, 결제, 업로드, Studio, 관리 기능도 모던 화면을 사용합니다. 상세 범위는 [기능표](docs/features.md)를 확인하세요.
...
`ie.zuzunza.com`처럼 전용 Classic 호스트를 운영할 때는 `ZUKU_PUBLIC_ORIGIN=http://ie.zuzunza.com`과 `ZUKU_FORCE_CLASSIC=1`을 함께 설정합니다. 이 모드에서는 접속한 브라우저 종류와 관계없이 같은 호스트의 `/hype`, `/swipe`, `/vine`, `/vive` 경로를 사용하며 제품 서브도메인으로 이동하지 않습니다. 평문 HTTP 전용 호스트에서는 공개 열람만 동작합니다.
