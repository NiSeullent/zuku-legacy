...
export function createLegacyApp(options: LegacyOptions) {
  ...
  async function handle(request: Request, context: RequestContext = {}): Promise<Response> {
    ...
    try {
      ...
      if (ctx.path === '/connect') {
        if (!ctx.secure) fail(404, '페이지를 찾을 수 없습니다.');
      ...
      if (ctx.path === '/messages') fail(404, '페이지를 찾을 수 없습니다.');
      ...
      if (ctx.path === '/thread') {
        ...
        const composer=v.form(ctx,'post','게시하기',{}, {label:'지금 무슨 생각을 하나요?',max:280});
        return html('Thread',v.intro('지금, 나누고 싶은 이야기','짧은 글로 이어지는 ZUKU의 일상.')+v.actionPanel(ctx,'글 쓰기',composer)+data.posts.map(p=>v.threadCard(ctx,p)).join('')+(data.next_cursor ? `<p><a class="lc-button" href="${e(v.href(ctx,'/thread',{cursor:data.next_cursor}))}">다음 이야기</a></p>`:''));
      }
      if (ctx.path.startsWith('/thread/')) {
        ...
        const reply=v.form(ctx,'reply','답글 남기기',{id},{label:'답글',max:280});
        return html('대화',v.intro('Thread','하나의 이야기, 이어지는 대화.')+data.posts.map(p=>v.threadCard(ctx,p,true)).join('')+v.actionPanel(ctx,'대화에 참여하기',reply)+(data.next_cursor?`<p><a class="lc-button" href="${e(v.href(ctx,'/thread/'+id,{cursor:data.next_cursor}))}">다음 답글</a></p>`:''));
      }
      ...
      return html(status === 404 ? '페이지를 찾을 수 없습니다' : '요청을 완료하지 못했습니다',v.intro(status === 404 ? '페이지를 찾을 수 없습니다' : '잠시 확인이 필요해요',message)+v.panel('계속 둘러보기',v.link(ctx,'','홈으로','lc-button')),status);
    }
